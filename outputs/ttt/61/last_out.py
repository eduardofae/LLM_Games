...
def jdv():
  ...
  # Create a simple grid representation for the game board.
  grid = [[' ', ' ', ' '], [' ', ' ', ' '], [' ', ' ', ' ']]
  ...
  while True:
    try:
      ...
      grid[move[0]][move[1]] = 'X' if players[current_player] == 1 else 'O'
...
def print_grid(grid):
  """
  Prints the game board in a grid format.

  Args:
    grid: The game board represented as a list of lists.
  """
