...
def check_winner(grid):
    ...
    for col in range(3):
        column = grid_np[:, col]
        if np.all(column == column[0]) and column[0] != " ":
            return column[0]

    # Check for diagonal wins
    diagonals = [
        [grid_np[0, 0], grid_np[1, 1], grid_np[2, 2]],
        [grid_np[0, 2], grid_np[1, 1], grid_np[2, 0]],
    ]
    for diagonal in diagonals:
        if np.all(diagonal == diagonal[0]) and diagonal[0] != " ":
            return diagonal[0]
